...
class patientProfileActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        findPatientDetails(patientID,RetrofitClient?.user?.DoctorProfile?.email)
    }

    private fun findPatientDetails(patientID: Int, email: String?) {
        ...
        try {
            Log.i("details" ,  """
                ...
                ${patientID}
            """.trimIndent())
            Ion.with(this).load("http://13.124.117.70/api/v1.0/DoctorPatientProfile")
                    ...
                    .asJsonObject().setCallback(object : FutureCallback<JsonObject>{
                        override fun onCompleted(e: Exception?, result: JsonObject?) {
                            val response = Gson().fromJson<patientProfile>(result,patientProfile::class.java)
